    ...
    var $animateMinErr = minErr("$animate"), ELEMENT_NODE = 1, NG_ANIMATE_CLASSNAME = "ng-animate", $$CoreAnimateRunnerProvider = function() {
        ...
        this.$get = [ "$$AnimateRunner", "$rootScope", function($$AnimateRunner, $rootScope) {
            function addRemoveClassesPostDigest(element, add, remove) {
                ...
                }, classesAdded = updateData(add, !0), classesRemoved = updateData(remove, !1);
                !classesAdded && !classesRemoved || postDigestElements.length > 1 || $rootScope.$$postDigest(function() {
                ...
                compile: function(formElement, attr) {
                    ...
                    return {
                        pre: function(scope, formElement, attr, controller) {
                            ...
                            var parentFormCtrl = controller.$$parentForm, setter = nameAttr ? getSetter(controller.$name) : noop;
                            nameAttr && (setter(scope, controller), attr.$observe(nameAttr, function(newValue) {
                                controller.$name !== newValue && (setter(scope, undefined), parentFormCtrl.$$renameControl(controller, newValue), 
                                (setter = getSetter(controller.$name))(scope, controller));
                            })), formElement.on("$destroy", function() {
                                parentFormCtrl.$removeControl(controller), setter(scope, undefined), extend(controller, nullFormCtrl);
                            });
                        }
                    };
                }
...
function(window, angular, undefined) {
    ...
    if (angular.mock.TzDate = function(offset, timestamp) {
        ...
        }), $provide.decorator("$animate", [ "$delegate", "$timeout", "$browser", "$$rAF", "$$forceReflow", function($delegate, $timeout, $browser, $$rAF, $$forceReflow) {
            var animate = {
                queue: [],
...
function(window, angular, undefined) {
    ...
    "use strict";
    "function" == typeof define && define.amd ? define([ "angular" ], factory) : "object" == typeof exports ? module.exports = factory(require("angular")) : factory(root.angular);
    ...
    function _storageProvider(storageType) {
        return function() {
            ...
            this.setSerializer = function(s) {
                ...
                var _last$storage, _debounce, prefixLength = storageKeyPrefix.length, webStorage = isStorageSupported(storageType) || ($log.warn("This browser does not support Web Storage!"), 
                {
                ...
                return $storage.$sync(), _last$storage = angular.copy($storage), $rootScope.$watch(function() {
                    ...
                    storageKeyPrefix === event.key.slice(0, prefixLength) && (event.newValue ? $storage[event.key.slice(prefixLength)] = deserializer(event.newValue) : delete $storage[event.key.slice(prefixLength)], 
