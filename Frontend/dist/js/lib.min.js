...
function(window, angular, undefined) {
    ...
    function _storageFactory(storageType) {
        return [ "$rootScope", "$window", "$log", "$timeout", function($rootScope, $window, $log, $timeout) {
            ...
            var _last$storage, _debounce, webStorage = isStorageSupported(storageType) || ($log.warn("This browser does not support Web Storage!"), 
            {
                setItem: function() {},
                getItem: function() {}
            }), $storage = {
            ...
            try {
                webStorage = $window[storageType], webStorage.length;
            } catch (e) {
                $log.warn("This browser does not support Web Storage!"), webStorage = {};
            }
            for (var k, i = 0, l = webStorage.length; l > i; i++) (k = webStorage.key(i)) && "ngStorage-" === k.slice(0, 10) && ($storage[k.slice(10)] = angular.fromJson(webStorage.getItem(k)));
            return _last$storage = angular.copy($storage), $rootScope.$watch(function() {
                ...
                _debounce || (_debounce = $timeout(function() {
                    if (_debounce = null, !angular.equals($storage, _last$storage)) {
                        ...
                        for (var k in temp$storage) webStorage.removeItem("ngStorage-" + k);
