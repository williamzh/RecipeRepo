...
recipeRepoApp.config([ "$httpProvider", function($httpProvider) {
    ...
    $scope.createSections = function() {
        apiClient.getRecipes().then(function(recipes) {
            var filteredRecipes = recipes.findAll(function(r) {
                return !r.isPrivate;
            });
    ...
    $scope.init = function() {
        ...
        apiClient.getUser(userName).then(function(user) {
            $scope.profile = user, $scope.fakePassword = "********";
        })["catch"](function() {
            $scope.showError = !0;
        });
    }, $scope.updateProfile = function() {
        profileForm.$invalid || ($scope.profileUpdated = !1, apiClient.updateUser($scope.profile.userName, $scope.profile).then(function() {
            $scope.profileUpdated = !0, $scope.recipeForm.$setPristine(), $scope.submitted = !1;
        })["catch"](function() {
            $scope.showError = !0;
        }));
    }, $scope.hasError = function(field) {
    ...
    $scope.previousState = $stateParams.referrer || "home.start", $scope.deleteModalHeading = localizationService.translate("recipeDetails", "confirmRemoveHeading"), 
...
recipeRepoDirectives.directive("rdAlert", [ "$window", function($window) {
    ...
    this.login = function(userName, password) {
        ...
        return $http.get(apiUrl + "/user/" + userName).then(function(response) {
            return onSuccess("getUser", response);
        })["catch"](function(error) {
            return onError("getUser", error);
        });
